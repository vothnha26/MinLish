...
- **Background Tasks:** Android WorkManager (scheduling local daily study reminders)
...
### 3.6. Chức năng Kế hoạch Học tập Hàng ngày (Daily Learning Plan) - Mới!

- **Thiết lập mục tiêu ngày:** Hỗ trợ lưu trữ cấu hình số lượng từ mới học (`dailyNewWordsTarget`) và số lượng từ cần ôn tập (`dailyReviewWordsTarget`) trong mỗi `UserProfile` trên Firestore.
- **Lập lịch phiên học thông minh:** Viết thuật toán `getDailySessionWords` tự động kết hợp các từ đến hạn ôn tập (Due) qua Spaced Repetition (SM-2) và các từ mới (New) để lấp đầy hạn mức ngày của người dùng.
- **Giao diện HomeScreen động:** Kết nối `HomeViewModel` hiển thị trực tiếp thanh tiến trình Progress Bar ngày (ví dụ: `6/30 words`), chỉ số đếm số từ còn lại, số từ đã thuộc tổng cộng và tỷ lệ ghi nhớ Accuracy dựa trên Ease Factor thực tế.

### 3.7. Hệ thống Thông báo (Notification System) - Mới!

- **Thông báo nhắc học cục bộ (WorkManager):** Tích hợp `ReminderWorker` và `NotificationHelper` để lập lịch gửi thông báo đẩy nhắc học mỗi 24 giờ vào đúng khung giờ người dùng lựa chọn trong màn hình Settings.
- **Đăng tải Thông báo Hệ thống (Admin):** Màn hình `AdminNotificationScreen` cho phép quản trị viên đăng tải thông báo hệ thống lên Firestore collection `notifications`.
- **Hộp thư Thông báo (User):** Màn hình `NotificationListScreen` tải danh sách toàn bộ thông báo hệ thống trực tiếp từ Firestore cho học viên theo dõi.
